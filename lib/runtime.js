...
async function init() {
  ...
  try {
    ...
    // 启动首刷是全量（含自有站）；把自有站节流计时也置为此刻，
    // 避免用户马上打开「我的站点」时又立刻多刷一次自己的生产站
    rt._ownRefreshedAt = Date.now();
    refreshAll(rt, { scope: "all" }).catch(() => {});
    restartPolling(rt);
  } catch (err) {
  ...
  console.log(`\n  fine v2 已就绪（${(process.env.DB_DRIVER || "sqlite").toLowerCase() === "mysql" ? "MySQL" : `SQLite: ${resolveDbPath()}`}）\n`);
  return rt;
}
