...
function buildMessage(from, toList, subject, text, html) {
  const common = [
    ...
    `Message-ID: <${Date.now()}.${Math.random().toString(36).slice(2)}@fine-apihub>`,
    "MIME-Version: 1.0",
  ];
  if (!html) {
    return [
...
export async function sendSmtpMail(cfg, subject, text, html) {
  ...
  try {
    ...
    let caps = (await cmd("EHLO fine-apihub", [250])).text.toUpperCase();
    ...
    if (port !== 465 && caps.includes("STARTTLS")) {
      ...
      attach(sock);
      caps = (await cmd("EHLO fine-apihub", [250])).text.toUpperCase();
    }
