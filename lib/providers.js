...
function unwrapSub2Data(body) {
  return body?.data ?? body;
}

function listItems(body) {
  const d = unwrapSub2Data(body);
  return Array.isArray(d) ? d : (d?.items || d?.list || d?.users || d?.api_keys || []);
}

/** Query Sub2API administrator API-key usage without exposing key material. */
export async function querySub2ApiKeyUsage(station, { startMs, endMs, pageSize = 1000 } = {}) {
  const base = trimBase(station.baseUrl);
  if (!base) throw new Error("缺少站点地址");
  const bearer = await sub2apiBearer(station, base);
  const headers = { Authorization: `Bearer ${bearer}` };
  const req = (url, opts = {}) => request(url, { headers, timeoutMs: 20000, ...opts });
  const usersRes = await req(`${base}/api/v1/admin/users?page=1&page_size=1000`);
  if (usersRes.status === 401 || usersRes.status === 403) throw new Error("Sub2API 令牌没有管理员权限");
  if (usersRes.status >= 300) throw new Error(httpErrorMessage(usersRes));
  const users = listItems(usersRes.body);
  const keys = [];
  for (const u of users) {
    const uid = Number(u.id ?? u.user_id);
    if (!Number.isFinite(uid) || uid <= 0) continue;
    const kr = await req(`${base}/api/v1/admin/users/${uid}/api-keys?page=1&page_size=${pageSize}`);
    if (kr.status === 401 || kr.status === 403) throw new Error("Sub2API 令牌没有管理员权限");
    if (kr.status >= 300) throw new Error(httpErrorMessage(kr));
    for (const k of listItems(kr.body)) keys.push({ id: Number(k.id), name: String(k.name || k.key_name || ""), userId: uid, username: String(u.username || u.name || "") });
  }
  const ids = [...new Set(keys.map((k) => k.id).filter((id) => Number.isFinite(id) && id > 0))];
  if (!ids.length) return { items: [], summary: { actualCost: 0, todayCost: 0, coveragePct: 100 }, source: "sub2api-admin" };
  const body = { api_key_ids: ids };
  const usage = await req(`${base}/api/v1/admin/dashboard/api-keys-usage`, { method: "POST", json: body });
  if (usage.status === 401 || usage.status === 403) throw new Error("Sub2API 令牌没有管理员权限");
  if (usage.status >= 300) throw new Error(httpErrorMessage(usage));
  const stats = unwrapSub2Data(usage.body)?.stats || {};
  const byId = new Map(keys.map((k) => [k.id, k]));
  const items = ids.map((id) => {
    const k = byId.get(id); const s = stats[id] || stats[String(id)] || {};
    const input = num(s.total_input_tokens ?? s.input_tokens);
    const output = num(s.total_output_tokens ?? s.output_tokens);
    const cache = num(s.total_cache_tokens ?? s.cache_tokens);
    return { ...k, requests: num(s.total_requests ?? s.requests), inputTokens: input, outputTokens: output, cacheTokens: cache, totalTokens: input + output + cache, actualCost: num(s.total_actual_cost ?? s.actual_cost), totalCost: num(s.total_cost), todayCost: num(s.today_actual_cost ?? s.today_cost) };
  }).sort((a, b) => b.actualCost - a.actualCost);
  const actualCost = items.reduce((n, x) => n + x.actualCost, 0);
  return { items, summary: { actualCost, todayCost: items.reduce((n, x) => n + x.todayCost, 0), coveragePct: 100 }, effectiveRange: "30d-or-provider-default", source: "sub2api-admin" };
}

// ---------------------------------------------------------------------------
// 按日花费（月历 / 每日消耗）—— sub2api 与 new-api 两类站点的统一入口
// ---------------------------------------------------------------------------
// 实测口径（2026-09-17，对 api.usora.net + pureai.ink 实测）：
//   · new-api  POST/GET /api/data/self → 行按 (model_name, created_at) 聚合，created_at 是**小时**对齐的
//     Unix 秒（相邻差值 3600）。硬限制：**时间跨度不得超过 1 个月**（31 天即 success:false）；
//     实际可查保留约 20 天（476 小时）。→ 因此必须把窗口切成 ≤30 天的片再合并。
//   · Sub2API  /api/v1/usage/dashboard/snapshot-v2?granularity=day|hour 直接给自然日/小时行，
//     花费字段是 actual_cost（与站点面板一致）；但只返回**有数据**的日期，且保留约 23~30 天。
// 统一返回 { days, hours, todayUsd, coverage, source }：
//   days  = [{ date: "YYYY-MM-DD"(按 tz), usd, tokens, requests }]（升序，仅含有数据的日）
//   hours = [{ t: ms, usd })（升序，仅当窗口含「今天」时返回今天的逐小时）
const MAX_NEWAPI_SPAN_DAYS = 30; // 上游硬限制「时间跨度不能超过 1 个月」，留 1 天余量

/** 把 [startMs, endMs) 切成每片不超过 maxDays 天的窗口（用于绕开 new-api 的跨度上限）。 */
export function chunkRange(startMs, endMs, maxDays = MAX_NEWAPI_SPAN_DAYS) {
  const out = [];
  const step = maxDays * 86400000;
  for (let s = startMs; s < endMs; s += step) {
    out.push([s, Math.min(s + step, endMs)]);
  }
  return out.length ? out : [[startMs, endMs]];
}

/** new-api：拉小时行 → 按 tz 分桶成「日」+「今天的小时」。 */
async function newApiDailyCost(station, { startMs, endMs, tz }) {
  const base = trimBase(station.baseUrl);
  const token = String(station.accessToken || "").trim();
  ...
  const days = new Map();
  const hours = [];
  const todayLabel = dateStrInTz(Date.now(), tz);
  let earliest = null;

  for (const [s, e] of chunkRange(startMs, endMs)) {
    const w = unixSecondWindow(s, e);
    const r = await request(`${base}/api/data/self?start_timestamp=${w.start}&end_timestamp=${w.end}`, { headers });
    if (r.status >= 300) throw new Error(httpErrorMessage(r));
    // success:false 时把上游原话透出（例如「时间跨度不能超过 1 个月」），不要一律归因于「未开启数据看板」
    if (r.body?.success === false) throw new Error(r.body?.message || "站点未开启数据看板（DataExportEnabled）");
    const rows = Array.isArray(r.body?.data) ? r.body.data : [];
    for (const row of rows) {
      const ms = num(row.created_at) * 1000;
      if (!ms) continue;
      const usd = num(row.quota) / QUOTA_PER_UNIT;
      const label = dateStrInTz(ms, tz);
      if (earliest == null || label < earliest) earliest = label;
      const d = days.get(label) || { date: label, usd: 0, tokens: 0, requests: 0 };
      d.usd += usd;
      d.tokens += num(row.token_used);
      d.requests += num(row.count);
      days.set(label, d);
      if (label === todayLabel) hours.push({ t: ms, usd });
    }
  }
  return { days, hours, coverageFrom: earliest, source: "newapi:/api/data/self" };
}

/** Sub2API：直接取自然日行；今天的小时另取一次 granularity=hour。 */
async function sub2ApiDailyCost(station, { startMs, endMs, tz }) {
  const base = trimBase(station.baseUrl);
  if (!base) throw new Error("缺少站点地址");
  const bearer = await sub2apiBearer(station, base);
  const call = async (path, params) => {
    const qs = new URLSearchParams(params);
    const r = await request(`${base}${path}?${qs}`, { headers: { Authorization: `Bearer ${bearer}` }, timeoutMs: 20000 });
    return r;
  };
  const dayParams = {
    start_date: dateStrInTz(startMs, tz),
    end_date: dateStrInTz(Math.max(startMs, endMs - 1), tz),
    granularity: "day",
    timezone: tz,
  };
  const r = await call("/api/v1/usage/dashboard/snapshot-v2", dayParams);
  if (r.status >= 300) throw new Error(httpErrorMessage(r));
  if (r.body?.code !== 0) throw new Error(r.body?.message || "Sub2API 用量接口返回失败");
  const rows = r.body?.data?.trend || [];
  const days = new Map();
  let earliest = null;
  for (const p of rows) {
    const label = String(p.date || "").slice(0, 10);
    if (!/^\d{4}-\d{2}-\d{2}$/.test(label)) continue;
    if (earliest == null || label < earliest) earliest = label;
    const d = days.get(label) || { date: label, usd: 0, tokens: 0, requests: 0 };
    d.usd += num(p.actual_cost ?? p.cost);
    d.tokens += num(p.total_tokens);
    d.requests += num(p.requests);
    days.set(label, d);
  }

  // 今天的逐小时（用于「实时消耗」）；只查今天这一天
  let hours = [];
  const todayLabel = dateStrInTz(Date.now(), tz);
  if (dateStrInTz(endMs - 1, tz) >= todayLabel && dateStrInTz(startMs, tz) <= todayLabel) {
    try {
      const hr = await call("/api/v1/usage/dashboard/snapshot-v2", {
        start_date: todayLabel, end_date: todayLabel, granularity: "hour", timezone: tz,
      });
      if (hr.status < 300 && hr.body?.code === 0) {
        hours = (hr.body?.data?.trend || []).map((p) => ({
          t: parseDateLabel(p.date, tz),
          usd: num(p.actual_cost ?? p.cost),
        })).filter((p) => p.t != null);
      }
    } catch { /* 小时曲线拿不到不算失败，日粒度仍可用 */ }
  }
  return { days, hours, coverageFrom: earliest, source: "sub2api:snapshot-v2" };
}

/**
 * 查询某站点的按日花费（月历用）与今天的实时小时花费。
 * 不支持的站点类型返回 available:false + reason，由调用方在 UI 上如实标注。
 */
export async function queryDailyCost(station, { startMs, endMs, tz }) {
  const type = station.type;
  if (type === "newapi-key") {
    return { available: false, reason: "sk 密钥模式没有用量接口", days: [], hours: [], source: null };
  }
  if (type === "fixed") {
    return { available: false, reason: "固定成本站点不产生用量", days: [], hours: [], source: null };
  }
  let raw;
  if (type === "newapi") raw = await newApiDailyCost(station, { startMs, endMs, tz });
  else if (type === "sub2api" || type === "sub2api-password") raw = await sub2ApiDailyCost(station, { startMs, endMs, tz });
  else return { available: false, reason: `未知的站点类型：${type}`, days: [], hours: [], source: null };

  const days = [...raw.days.values()]
    .map((d) => ({ date: d.date, usd: Math.round(d.usd * 1000000) / 1000000, tokens: d.tokens, requests: d.requests }))
    .sort((a, b) => (a.date < b.date ? -1 : a.date > b.date ? 1 : 0));
  const hours = raw.hours.sort((a, b) => a.t - b.t);
  const todayLabel = dateStrInTz(Date.now(), tz);
  const today = days.find((d) => d.date === todayLabel);
  return {
    available: true,
    days,
    hours,
    todayUsd: today ? today.usd : 0,
    // 上游保留期有限：最早可查日期（用户据此判断「历史月份为何空」）
    coverageFrom: days.length ? days[0].date : raw.coverageFrom || null,
    source: raw.source,
  };
}
 ...
 * 返回 [{user, group, model, channelId, channelName, tokenName, tokens, cost($), requests}]
 */
export async function queryOwnFlow(station, startMs, endMs) {
  const base = trimBase(station.baseUrl);
  const token = String(station.accessToken || "").trim();
  if (!base || !token) throw new Error("缺少站点地址或访问令牌");
  const headers = { Authorization: token };
